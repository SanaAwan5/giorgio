...
class Alice:
    ...
    def __init__(self):
         ...
         self.model=MLPClassifier(hidden_layer_sizes=(200,200), max_iter=70, alpha=1e-2,
    ...
    def decrypt_aggregate(self, en_aggr, n_clients):
        clf = self.model
        decrypted_res=[]
        for i in range (len(clf.coefs_)):
            ...
            for j in range(clf.coefs_[i].shape[1]):
                 list1 = decrypt_vector(self.privkey, en_aggr[i][j])
                 list2.append(list1)
            decrypted_res.append(list2)
...
class Bob:
    ...
    def encrypt_weights(self, weights):
        coef = weights
        print("coef.shape[0]", coef.shape[0])
...
def federated_learning(X, y, X_test, y_test, X_valid, y_valid, config):
    ...
    for i in range(n_iter):
        ...
        for c in clients_r:
            ...
            for i in range(len(clf.coefs_)):
                list1=[]
                list2=[]
                print("length", len(clf.coefs_))
                number_neurons_in_layer = clf.coefs_[i].shape[1]
                print("num neurons in layer %s %s,", i,number_neurons_in_layer)
                for j in range(number_neurons_in_layer):
                    weights = clf.coefs_[i][:,j]
                    list1=encrypt_vector(c.pubkey,clf.coefs_[i][:,j])
                    #en_weights.append(encrypted_weights)
                    list2.append(list1)
                vec.append(list2)
            print("len(vec)",len(vec[1]))
            c.weights = np.asarray(vec)
        #en_aggr=clients[0].weights
        clf = clients_r[0].model
        for i in range(len(clf.coefs_)):
            en_aggr =np.asarray(vec)
            number_neurons_in_layer = clf.coefs_[i].shape[1]
            for j in range(number_neurons_in_layer):
                #en_aggr[i][j] is a list 
                en_aggr[i][j]=clients_r[0].weights[i][j]
                #print("len len len", vec[i][j])
                for k in range(1,nr_clients - 1):
                    #en_aggr = list(clients[0].weights[i][:,j])
                    #en_aggr2 = list(clients[k].weights[i][:,j])
                    en_aggr[i][j] = sum_encrypted_vectors(en_aggr[i][j],np.asarray(clients_r[k].weights[i][j])) 
        #aggr = alice.decrypt_aggregate(en_aggr, n_clients)
        #aggr = np.asarray(aggr)
        clf = alice.model
        for i in range(len(clf.coefs_)):
            number_neurons_in_layer = clf.coefs_[i].shape[1]
            print("num neurons in layer %s %s,", i,number_neurons_in_layer)
            for j in range(number_neurons_in_layer):
                aggr = decrypt_vector(alice.privkey,np.asarray(en_aggr[i][j]))
                aggr = np.multiply(aggr, config['eta']) / 10
                weights = clf.coefs_[i][:,j]
                #print(weights)
                alice.model.coefs_[i][:,j] = clf.coefs_[i][:,j] - aggr
                #alice.model.coefs_[i][:,j]= aggr[i][j]
    ...
    #for c in clients:
        ...
        #print(alice.fit(X_test, y_test))
        print("Alice fitting after...")
        alice.fit(X_test, y_test)
